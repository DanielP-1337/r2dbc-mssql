...
/**
 * Integration tests for NVARCHAR(MAX) cells in table-valued parameters.
 */
class NvarcharMaxTableValuedParameterIntegrationTests extends IntegrationTestSupport {
    ...
    void prepareSchema() {
        ...
        SERVER.getJdbcOperations().execute(
                "CREATE TYPE dbo.r2dbc_tvp_nvarchar_max_table AS TABLE " +
                        "(ordinal INT NOT NULL, value NVARCHAR(MAX) NULL)");

        SERVER.getJdbcOperations().execute(
                "CREATE PROCEDURE dbo.r2dbc_tvp_nvarchar_max_echo " +
                        "@values dbo.r2dbc_tvp_nvarchar_max_table READONLY AS " +
    ...
    @Test
    void shouldRoundTripUnicodeEmptyNullAndLargeValueWithIntegerColumn() {
    ...
    private static void assertUnicodeRoundTrip(MssqlConnection testConnection) {
        ...
        String unicode = "A\u00e4\u6f22\ud83d\ude0a";
        String largeValue = String.join("", Collections.nCopies(32768, "x")) + "\ud83d\ude0a";
        ...
        assertRows(testConnection, table,
                "1:" + unicode, "2:", "3:null", "4:" + largeValue, "5: trailing spaces  ");
    }

    private static MssqlTableValue.Builder nvarcharTable() {
        return MssqlTableValue.builder("dbo.r2dbc_tvp_nvarchar_max_table")
    ...
    private static void assertRows(MssqlConnection testConnection, MssqlTableValue table, String... expected) {
        ...
        testConnection.createStatement("EXEC dbo.r2dbc_tvp_nvarchar_max_echo @values")
                .bind("@values", table)
                .execute()
    ...
    private static void dropSchema() {
        ...
        SERVER.getJdbcOperations().execute(
                "IF OBJECT_ID(N'dbo.r2dbc_tvp_nvarchar_max_echo', N'P') IS NOT NULL " +
                        "DROP PROCEDURE dbo.r2dbc_tvp_nvarchar_max_echo");

        SERVER.getJdbcOperations().execute(
                "IF TYPE_ID(N'dbo.r2dbc_tvp_nvarchar_max_table') IS NOT NULL " +
                        "DROP TYPE dbo.r2dbc_tvp_nvarchar_max_table");
    }
}
